...
openssl req -new -x509 -keyout ca-key.pem -out ca-cert.pem -days 365 -subj "/CN=kafka-ssl-CA" -passout pass:"$ssl_pass"
...
do
   ...
   keytool -importkeystore -srckeystore "$i".p12 -srcstoretype PKCS12 -srcstorepass "$keystore_pass" -destkeystore "$i".keystore.jks -deststorepass "$keystore_pass"
...
keytool -importkeystore -srckeystore client.p12 -srcstoretype PKCS12 -srcstorepass "$client_keystore_pass" -destkeystore client.keystore.jks -deststorepass "$client_keystore_pass"
...
#sed -i "s|ssl.truststore.password=.*$|ssl.truststore.password=\$truststore_pass\|g" ../client.properties
#sed -i "s|ssl.keystore.password=.*$|ssl.keystore.password=\$client_keystore_pass\|g" ../client.properties
#sed -i "s|ssl.key.password=.*$|ssl.key.password=\$ssl_pass\|g" ../client.properties
